"use client";

import { useEffect, useState } from "react";
import Link from "next/link";
import { usePathname, useRouter } from "next/navigation";

/** Names that get the admin shortcut surfaced next to the home icon.
    Anyone else can still reach /admin by URL (the gate is the password,
    not the visibility of this link). */
const ADMIN_NAMES = new Set(["Aaron"]);

export function Header() {
  const path = usePathname();
  const router = useRouter();
  // trailingSlash: true in next.config means pathnames come in as "/changelog/",
  // so strip the trailing slash before matching.
  const p = path.replace(/\/$/, "");
  const onOrders = p.endsWith("/orders");
  const onHome = p === "";
  const onAdmin = p.endsWith("/admin");

  // Re-read the cached name on every navigation so the gear appears /
  // disappears when the user picks a different name on the home page.
  const [cachedName, setCachedName] = useState<string | null>(null);
  useEffect(() => {
    try { setCachedName(localStorage.getItem("hellokopi_name")); } catch {}
  }, [path]);
  // Admin gear is suppressed on the home page — the greeting screen is for
  // picking a name, not jumping into admin. Aaron still gets the shortcut
  // on every other route.
  const showAdminLink = !onAdmin && !onHome && cachedName !== null && ADMIN_NAMES.has(cachedName);

  return (
    <>
      {/* Top-left cluster: home + (Aaron-only) admin gear */}
      {!onHome && (
        <div className="fixed top-0 left-0 z-50 p-5 sm:p-6 flex items-center gap-3">
          {!onHome && (
            <Link
              href="/"
              aria-label="Home"
              className="text-stone-400 hover:text-stone-600 dark:text-stone-500 dark:hover:text-stone-300 transition-colors duration-200 touch-manipulation focus:outline-none"
            >
              <svg className="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
              </svg>
            </Link>
          )}
          {showAdminLink && (
            <Link
              href="/admin"
              aria-label="Admin"
              className="text-stone-400 hover:text-stone-600 dark:text-stone-500 dark:hover:text-stone-300 transition-colors duration-200 touch-manipulation focus:outline-none"
            >
              <svg className="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.431l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28Z" />
                <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
              </svg>
            </Link>
          )}
        </div>
      )}

      {/* Top-right: orders nav. The admin shortcut used to live here on the
          changelog page; it's now folded into the top-left cluster above. */}
      <div className="fixed top-0 right-0 z-50 p-5 sm:p-6">
        {onOrders ? (
          <button
            onClick={() => router.back()}
            className="text-[10px] uppercase tracking-[0.25em] text-stone-400 hover:text-stone-700 dark:text-stone-500 dark:hover:text-stone-200 transition-colors duration-200 font-sans font-medium touch-manipulation"
          >
            ← Place Order
          </button>
        ) : !onAdmin && (
          <Link
            href="/orders"
            className="text-[10px] uppercase tracking-[0.25em] text-stone-400 hover:text-stone-700 dark:text-stone-500 dark:hover:text-stone-200 transition-colors duration-200 font-sans font-medium touch-manipulation"
          >
            Check Orders →
          </Link>
        )}
      </div>
    </>
  );
}
