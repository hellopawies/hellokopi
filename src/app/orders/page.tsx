...
function WhatsAppShareButton({ session, dateLabel }: { session: Session; dateLabel: string }) {
  const [copied, setCopied] = useState(false);

  function handleShare() {
    const text = buildShareText(session, dateLabel);
...
function buildCollectorText(sessions: Session[], label: string): string {
  ...
  const cups = sorted.reduce((s, [, n]) => s + n, 0);
  const lines = [
    `☕ Drinks Order - ${label}`,
    "",
    ...sorted.map(([drink, qty]) => qty > 1 ? `${drink} × ${qty}` : drink),
    "",
    `${cups} ${cups === 1 ? "cup" : "cups"} total`,
  ];
  return lines.join("\n");
...
function CollectorView({ sessions, dateKey }: { sessions: Session[]; dateKey: string }) {
  const [copied, setCopied] = useState(false);
  ...
  const sorted = [...totals.entries()].sort((a, b) => b[1] - a[1]);
  const cups = sorted.reduce((s, [, n]) => s + n, 0);
  const label = new Date(dateKey + "T12:00:00+08:00").toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric", timeZone: TIMEZONE_SG });

  function handleShare() {
    const url = `https://wa.me/?text=${encodeURIComponent(buildCollectorText(sessions, label))}`;
    window.open(url, "_blank", "noopener,noreferrer");
  }

  async function handleCopy() {
    try {
      await navigator.clipboard.writeText(buildCollectorText(sessions, label));
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <span className="text-[11px] uppercase tracking-[0.25em] text-stone-400 dark:text-stone-500 font-sans">
          {cups} {cups === 1 ? "cup" : "cups"} total
        </span>
        <div className="flex items-center gap-1">
          <button onClick={handleShare} title="Share on WhatsApp" className="p-1.5 text-stone-400 hover:text-stone-600 dark:text-stone-500 dark:hover:text-stone-300 transition-colors duration-200 touch-manipulation focus:outline-none">
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
            </svg>
          </button>
          <button onClick={handleCopy} title="Copy order text" className="p-1.5 text-stone-400 hover:text-stone-600 dark:text-stone-500 dark:hover:text-stone-300 transition-colors duration-200 touch-manipulation focus:outline-none">
            {copied ? (
      ...
      <div className="border-t border-stone-100 dark:border-stone-800">
        {sorted.map(([drink, qty]) => (
          <div key={drink} className="flex items-center justify-between gap-4 py-3.5 border-b border-stone-100 dark:border-stone-800">
            <p className="text-sm font-sans font-medium text-stone-800 dark:text-stone-100">{drink}</p>
            {qty > 1 && (
              <span className="flex-shrink-0 px-2 py-0.5 border border-stone-200 dark:border-stone-600 rounded-full text-[10px] font-sans font-medium text-stone-500 dark:text-stone-400 tracking-wide">
                × {qty}
              </span>
            )}
          </div>
    ...
    <main className="relative min-h-[100dvh] bg-[#FAFAF8] dark:bg-black pb-16">
        ...
        <div className="px-5 sm:px-8 pt-12 sm:pt-6 pb-4">
          <div className="max-w-lg mx-auto">
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              {activeGroup && (
                <button
                  onClick={() => setCollectorMode((m) => !m)}
                  title={collectorMode ? "Session view" : "Collector view"}
                  className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full border transition-colors duration-200 touch-manipulation text-[10px] uppercase tracking-[0.15em] font-sans font-medium ${collectorMode ? "border-stone-800 dark:border-stone-100 text-stone-800 dark:text-stone-100" : "border-stone-200 dark:border-stone-700 text-stone-400 dark:text-stone-500 hover:border-stone-400 dark:hover:border-stone-500 hover:text-stone-600 dark:hover:text-stone-300"}`}
                >
                  <svg className="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <line x1="8" y1="6" x2="21" y2="6" /><line x1="8" y1="12" x2="21" y2="12" /><line x1="8" y1="18" x2="21" y2="18" />
                    <line x1="3" y1="6" x2="3.01" y2="6" /><line x1="3" y1="12" x2="3.01" y2="12" /><line x1="3" y1="18" x2="3.01" y2="18" />
                  </svg>
                  {collectorMode ? "Sessions" : "Summary"}
                </button>
              )}
            </div>
            {quip && (
      ...
      <div className="px-5 sm:px-8 pt-6 pb-16">
        <div className="max-w-lg mx-auto">
          ...
          {/* Collector summary for selected day */}
          {activeGroup && collectorMode && (
            <CollectorView sessions={activeGroup.sessions} dateKey={activeGroup.dateKey} />
          )}
