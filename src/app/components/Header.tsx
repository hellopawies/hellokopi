...
export function Header() {
  ...
  const onOrderMenu = path.endsWith("/order");
  const onHome = path === "/" || path === "";
  const onAdmin = path.endsWith("/admin");
  const selectedName = (searchParams.get("name") ?? "").trim().toLowerCase();
  const showAdminOnOrderMenu = onOrderMenu && selectedName === "aaron";

  return (
    <>
      {!onHome && (
        <div className="fixed top-0 left-0 z-50 p-5 sm:p-6">
          {showAdminOnOrderMenu ? (
            <Link
              href="/admin"
              aria-label="Admin"
